...
We are particularly interested in investigating scattering occurring perpendicular to the direction of illumination. Consider the example of Rayleigh scattering illustrated below. Incident light passes through a first polariser aligned parallel to the z-axis, such that the associated dipole oscillation occurs along the z-axis, producing scattered light predominantly in the x–y plane. When detecting light perpendicular to the illumination (parallel to the y-axis) above the sample, a second polariser can be used to control which polarisation components are transmitted, depending on its orientation. For example, if the second polariser is also aligned with the z-axis, the Rayleigh-scattered light is transmitted because its polarisation matches the second polariser's orientation, resulting in a brighter detected signal. However, if the second polariser is rotated vertically, it only allows light polarised parallel to the x-axis to pass through. Under these conditions, Rayleigh scattered light is suppressed because its polarisation is not aligned with the second polariser. Mie scattered light, in contrast, still contributes to the detected signal in both polariser configurations.
...
Sameples consist of sunflower pollen (Mie scattering signal), suspended in agarose with varying concentrations of Rayleigh scattering (TiO_2) and Mie scattering (polystyrene latex microspheres) background. Fish embryo specimen were later investigated.
