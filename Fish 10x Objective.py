...
def everything(rootdir, mask_rootdir, title, ymin=800, ymax=1000, xmin=1000, xmax=1200, individual_sub_plot=0):
    ...
    if mask_rootdir == 0:
        ...
        for i in range(len(averages)):
            ax = axs.flatten()[i]
            ax.imshow(selected_region[i], cmap='viridis', vmin=avrg_background_vmin, vmax=avrg_background_vmax)
            ax.set_title(f'{array_names[i]}')
        ...
        for i, names_comb in enumerate(combinations_list):
            ax = axs.flatten()[i]
            ax.imshow(subtractions[i], cmap='viridis', vmin=sub_vmin, vmax=sub_vmax)
        ...
        for i in range(len(selected_region)):
            ax = axs.flatten()[i]
            ax.imshow(selected_region[i], cmap='viridis', vmin=sub_background_vmin, vmax=sub_background_vmax)
            ax.set_title(f'{names_comb[0]} - {names_comb[1]}')
